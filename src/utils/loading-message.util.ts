...
export default class {
        ...
        ;(async () => {
            ...
            while (!this.isStopped) {
                ...
                if (isRemoving) {
                    ...
                    if (reactionToRemove) {
                        // eslint-disable-next-line no-await-in-loop
                        await reactionToRemove.users.remove(
                            this.msg.client.user!.id
                        )
                    } else {
                        isRemoving = false
                    }
                    ...
                    if (!this.isStopped) {
                        // eslint-disable-next-line no-await-in-loop
                        reactions.push(
                            await this.msg.react(icons[i % icons.length])
                        )
                    }
    ...
    async stop(str = 'u betcha'): Promise<Message> {
        ...
        const reactPromise =
            str && !wasAlreadyStopped
                ? (async () => {
                      await this.msg.fetch()
                      await Promise.all(
                          this.msg.reactions.cache.map(async (react) => {
                              if (react.me) {
                                  await react.users.remove(
                                      this.msg.client.user!.id
                                  )
                              }
                          })
